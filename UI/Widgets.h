#ifndef OOP_L5_WIDGETS_H
#define OOP_L5_WIDGETS_H
...
public:
    ...
    */
    static void printTitle(const string &title, TitleStyle tStyle = TITLE_CHAR) {
        int half = (ScooterUI::MAX_LINE_WIDTH - int(title.size()) - 2) / 2;
        string str(half, tStyle);
        str += ' ' + title + ' ';
        str += string(half, (tStyle));

        while (str.size() < ScooterUI::MAX_LINE_WIDTH)
            str += char(tStyle);
...
*/
    static void tableView(vector<Scooter> &list) {
        ...
        int spacing = (ScooterUI::MAX_LINE_WIDTH - charCount) / int(columnHeaders.size());
